...
async def notify_admins_new_order(
    ...
    # Для адресов с "/" (например, "ломова181/1") преобразуем в формат, который 2ГИС лучше понимает
    # Заменяем "181/1" на "181 корпус 1" для лучшего распознавания
    # Но сохраняем оригинальный адрес для отображения
    address_for_search = delivery_address
    # Если адрес содержит паттерн "число/число", заменяем на "число корпус число"
    address_for_search = re.sub(r'(\d+)/(\d+)', r'\1 корпус \2', address_for_search)
    
    # Кодируем адрес для поиска - все специальные символы должны быть закодированы
    address_encoded = quote(address_for_search, safe='')
    
    # Используем формат с путем - 2ГИС лучше обрабатывает адреса в пути
    ...
    # В ссылке показываем оригинальный адрес, но поиск идет по преобразованному
